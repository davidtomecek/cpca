...
preprocess_dataset <- function(path_data_left,path_data_right,path_mcp,subset_left,subset_right,fs_orig=1,fs_up=3){
  ...
  data_left <- foreach(j=1:nrow(data_left),.combine='rbind') %dofuture% {
  ...
  data_right <- foreach(j=1:nrow(data_right),.combine='rbind') %dofuture% {
...
save(list=c('hemi_left','hemi_right','veri_left','veri_right','Sets','CPCA','desc','Data_comb'),file=paste0(path_work,'workspace_cpca4set_opt_',dec,'_',gsub(pattern='-',replacement='',x=Sys.Date()),'.RData'))
